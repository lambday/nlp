...
if (len(sys.argv) < 5):
	print 'Usage:', sys.argv[0], '<train-corpus-dir> <test-corpus-dir> <wlut> <twlut>'
	sys.exit(1)
...
# Create a tag-word list (which tag appears with which word)
twordlist = []
for i in range(len(tagset)):
	twordlist.append([])
twordlist.append(["^"])
...
for corpus in corpusFiles:
	...
	# add to tag-word list
	for tagcount in tagindex:
		if bsword not in twordlist[tagcount[0]]:
			twordlist[tagcount[0]].append(bsword)
	...
	for i in range(len(corpus))[1:]:
		...
		# add to tag-word list
		for tagcount in tagindex:
			if curword not in twordlist[tagcount[0]]:
				twordlist[tagcount[0]].append(curword)
...
# For demo purpose
#############################################################################
# Create two look-up tables, wLookup[currentword] will contain a list 
# containing the most probable next words using model 1.
# twLookup[currenttag][currentword] will contain another list containing 
# the most probable next words for model 2
wLookup = {}
twLookup = []
for i in range(len(tagset)+1):
	twLookup.append({})

# Iterate over all words to fillup word-lookup table
for word, val in wordlist.items():
	wLookup[word] = []
	# Find the argmax using model 1
	try:
		wLookup[word].append(findMax(wtTable[word]))
	except KeyError:
		wLookup[word].append([])

# Iterate over all tag-word pair to fillup tagged-word lookup
for i in range(len(tagset)+1):
	for curword in twordlist[i]:
		marginal = {}
		for j in range(len(tagset)):
			if curword in twtTable[j]:
				for nextword, val in twtTable[j][curword].items():
					if nextword not in marginal:
						marginal[nextword] = 0.0
					marginal[nextword] += val * ttTable[i][j]
		twLookup[i][curword] = findMax(marginal)

# Write the word-lookup table into a file
try:
	lut = open(sys.argv[3],'w')
	for word, val in wLookup.items():
		lut.write(word+":")
		for w in val[0]:
			lut.write(w + " ")
		lut.write("\n")
...
# Write the word-lookup table into a file
try:
	lut = open(sys.argv[4],'w')
	for i in range(len(tagset)+1):
		for word, val in twLookup[i].items():
			lut.write(word+":")
			for w in val:
				lut.write(w + " ")
			lut.write(",")
	...
	print sys.argv[0],'couldn\'t open lookup table file',sys.argv[4],'for writing'
	sys.exit(5)
...
for corpus in corpusFiles:
	...
	try:
		total = 0.0
		for tag in tagindex:
			total += twtTable[tag[0]][BSW][bsword] #* ttTable[BST][tag[0]]
		prob_tws *= total
	except KeyError:
		prob_tws *= 0.0	#minval
	
	# Store the previous word and tagindex
	prevword = bsword
	prevtagi = tagindex
	...
	for i in range(len(corpus))[1:]:
		...
		try:
			total = 0.0
#			for prevtag in prevtagi:
			for tag in tagindex:
				total += twtTable[tag[0]][prevword][curword] #* ttTable[prevtag[0]][tag[0]]
			prob_tws *= total
		except KeyError:
			prob_tws *= 0.0	#minval
		...
		# At the end of each sentence, calculate the following
			...
			# Store the current word and tagindex
			prevword = curword
			prevtagi = tagindex
