...
wordTransitionTable = {}
transitionProbTable = []
# emission probability emissionProbT[S_{i}][O_{i}]
emissionProbT = []
# emission probability emissionProbTable[O_{i}][S_{i}]
emissionProbTable = {}
...
# With the option "-tagged"
	...
	for line in etable:
		if len(line.strip()) != 0:
	        	emissionProbT.append(dict(map(lambda x:(x.split(":")[0],x.split(":")[1]),line.strip().split(" "))) )
		else:	emissionProbT.append({})
	# Form the transpose of this table
	for tag in range(len(tagset)):
		for word,val in emissionProbT[tag].items():
...
if not(isdir( sys.argv[-1] )):
	...
	if sys.argv[1] == "-untagged":
		...
		try:
			...
			predictedword = predictUntagged( lastword )[0]
		except KeyError:
	...
	# In case the words are tagged
		...
		try:
			...
			predictedword = predictTagged( lasttag )[0]
