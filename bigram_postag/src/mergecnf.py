...
try:
	...
	cnffiles = filter( lambda x: x.split(".")[0].split("_")[-1] == "cnf" and x or None ,cnffiles)
except OSError:
...
for cnf in cnffiles:
        try:
                f = open( sys.argv[1] + "/result" + sys.argv[2] + "/cnf/" + cnf, "r" )
		...
		for i in range(len(tagset)):
			mergedMatrix[i] = map( lambda x: x[0] + x[1], zip(mergedMatrix[i], confusionMatrix[i]))
                ...
                print sys.argv[0],'Error reading cnf file'

# print the merged confusion matrix in a file
name = "./results/" + sys.argv[2] + "_cnf.csv"
cnf = open( name, "w" )
for i in range(len(tagset)):
