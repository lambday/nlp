...
# run the suffix probability calculator once
echo 'Training suffix probabilities...'
#python getsuffixprob.py $1 sfx
...
for (( i=0; i<$FOLD; ++i )); do
	...
	echo 'Training the tagger for fold '`expr $i + 1`'...'
	python traintagger.py ./trainset tmp1 tmp2
	# run the tag, evaluate and genstat scripts with ./testset as the corpus directory
	echo 'Testing the tagger for fold '`expr $i + 1`'...'
	python tagwords.py tmp1 tmp2 sfx ./testset ./results/conf$i.csv

	rm -rf ./testset/*
	rm -rf ./trainset/*
done
#mv locstat.csv globstat.csv
rm -rf testset data* trainset tmp1 tmp2
