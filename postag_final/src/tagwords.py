...
def suffixMarginal( word, i ):
	retval = 1.0 / len(suffix)
	for s in suffix:
		if search( s + "$", word ) != None:
			# match found
			retval += suffixProbTable[i][suffix.index(s)]
	return retval

if len( sys.argv ) < 4:
	print 'Usage:',sys.argv[0],'<transition-prob-file> <emission-prob-file> <suffix-table> \
[optional<test-corpus-dir>] [optional<confusion-matrix-csv(only if test-corpus-dir provided)]'
	sys.exit(1)
try:
	ttmp = open( sys.argv[1], "r" )
	etmp = open( sys.argv[2], "r" )
	stmp = open( sys.argv[3], "r" )
except IOError:
	print sys.argv[0],'Error: Files',sys.argv[1],'and/or',sys.argv[2],'and/or',sys.argv[3],'doesn\'t exist'
	sys.exit(2)
...
if len( sys.argv ) == 4:
	...
	for corpusfile in corpusFiles:
        	try:
	                c = open( join(sys.argv[4],corpusfile), "r" )
	        except IOError:
...
# read the suffix probability table
suffixProbTable = []
for line in stmp:
	suffixProbTable.append( map(lambda x:float(x),line.strip().split(" ")) )
...
#TODO CHECK
	for j in range(len(tagset)):
		...
		try:
			T1[0][j] = transitionProbTable[BST][BST][j] * emissionProbTable[word][j] #* suffixProb
		except KeyError:
			# for unknown words, all tags are assumed to have equal probability
			# as their emission probability
#			print 'newword',word,
#			print 'suffixprob',suffixProb
			suffixProb = suffixMarginal( word, j )
			T1[0][j] = transitionProbTable[BST][BST][j] * suffixProb #* (1.0/(len(tagset)))
...
#TODO CHECK
		...
		for k in range(len(tagset)):
			...
			if word in emissionProbTable:
				ep = emissionProbTable[word][k] #* suffixProb
			else:
#				print 'newword',word,
#				print 'suffixprob',suffixProb
				suffixProb = suffixMarginal( word, k )
				ep = suffixProb #* 1.0/(len(tagset))
