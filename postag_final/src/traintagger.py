...
if (len(sys.argv) < 4):
	print 'Usage:', sys.argv[0], '<corpus-dir> <trigram-file> <emission-file>'
	sys.exit(1)
...
try:
	...
	print sys.argv[0],'Couldn\'t open',sys.argv[2],'and/or',sys.argv[3],'and/or',sys.argv[4],'for writing'
	sys.exit(5)
...
for i in range(len(tagset) + 1):
	for j in range(len(tagset) + 1):
		ttmp.write(str(i) + " " + str(j) + ":")
		for k in range(len(tagset)):
			ttmp.write( str(ttTable[i][j][k]) + " " )
		ttmp.write( "\n" )
